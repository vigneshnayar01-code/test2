...
def load_attendance_data():
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            name = row['Name'] if 'Name' in row and pd.notna(row['Name']) else f"Employee {row['Fake_Id']}"
            ...
            cafeteria_hours = float(row.get('cafeteria_hours', 0)) if row.get('cafeteria_hours') else round(float(row.get('Avg_Cafeteria', 0)), 1)
            ooo_hours = float(row.get('avg_ooo_hours', 0)) if row.get('avg_ooo_hours') else round(float(row.get('Avg_OOO_hr', 0)), 1)
            office_hours = float(row.get('avg_office_hours', 0)) if row.get('avg_office_hours') else round(float(row.get('Avg_Office_hr', 0)), 1)
            break_hours = float(row.get('avg_break_hours', 0)) if row.get('avg_break_hours') else round(float(row.get('Avg_Break_hr', 0)), 1)
...
# Optional: Handle form submissions from login page
...
if __name__ == '__main__':
    ...
    print("  - Employee & Organization View: http://localhost:5000/employee-view.html")
    print("Available APIs:")
    print("  - All employees: http://localhost:5000/api/employees")
    print("  - Search employee: http://localhost:5000/api/search_employee?q=<query>")
    print("  - Organization data: http://localhost:5000/api/organization_data")
